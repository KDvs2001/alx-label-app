...
import { Play, Clock, ArrowRight, CheckCircle2, ShieldAlert } from 'lucide-react';
...
const PilotTestModal = ({ username, onComplete }) => {
    ...
    const handleNext = () => {
        ...
        if (currentTextIndex < PILOT_TEXTS.length - 1) {
    ...
    const finishTest = async (finalTotalTime) => {
        ...
        // Calculate metrics
        const totalWords = PILOT_TEXTS.join(' ').split(/\s+/).length;
                ...
                {step === 'test' && (
                    <div className="space-y-6">
                        <div className="flex justify-between items-center text-sm font-bold text-slate-400">
                            ...
                            <span>{currentTextIndex + 1} of {PILOT_TEXTS.length}</span>
                        </div>
                        ...
                        <div className="bg-slate-950 border border-slate-800 rounded-xl p-5 min-h-[140px] flex items-center text-slate-300 text-lg leading-relaxed shadow-inner">
                            {PILOT_TEXTS[currentTextIndex]}
                        </div>
                        
                        <button
                            onClick={handleNext}
                            className="w-full bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-3.5 px-6 rounded-xl flex items-center justify-center gap-2 transition"
                        >
